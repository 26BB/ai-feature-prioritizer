...
export function buildRicePrompt(features) {
  // Security: Escape backslashes/quotes and normalize line breaks to prevent prompt injection & template breakout
  ...
  const featureList = features
    .map((f, i) => `${i + 1}. Name: "${sanitizePromptInput(f.name)}"\n   Description: "${sanitizePromptInput(f.description)}"\n   Category: ${sanitizePromptInput(f.category)}`)
    .join('\n');
