...
export async function prioritizeFeatures({ features, userProvider, userApiKey }) {
  ...
  // 1. If User Provided BYOK Key for a specific provider (Security: skip if provider is 'auto' or invalid to prevent misrouting keys to wrong API endpoints)
  if (
    userApiKey &&
    userApiKey.trim() &&
    userProvider &&
    userProvider !== 'auto' &&
    Object.prototype.hasOwnProperty.call(PROVIDER_CONFIGS, userProvider)
  ) {
    const config = PROVIDER_CONFIGS[userProvider];
    try {
      ...
      return { features: result, source: `BYOK (${userProvider.toUpperCase()})` };
    } catch (err) {
      console.warn(`BYOK (${userProvider}) failed:`, err.message);
