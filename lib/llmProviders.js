...
async function callOpenAiCompatible({ baseURL, apiKey, model, prompt }) {
  ...
  const sanitized = scored.map((f) => {
    ...
    // Security: Clamp LLM numerical outputs to valid RICE ranges to prevent out-of-bounds values or DoS via tiny effort division
    const reach = Number.isFinite(Number(item.reach)) ? Math.max(1, Math.min(10, Number(item.reach))) : 5;
    const impact = Number.isFinite(Number(item.impact)) ? Math.max(1, Math.min(10, Number(item.impact))) : 5;
    const confidence = Number.isFinite(Number(item.confidence)) ? Math.max(10, Math.min(100, Number(item.confidence))) : 80;
    const effort = Number.isFinite(Number(item.effort)) ? Math.max(1, Math.min(10, Number(item.effort))) : 3;
