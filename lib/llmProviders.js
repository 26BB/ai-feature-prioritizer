...
async function callOpenAiCompatible({ baseURL, apiKey, model, prompt }) {
  ...
  // Security & Data Integrity: Validate and sanitize untrusted LLM output fields
  const sanitized = scored.map((f) => {
    const reach = Number.isFinite(Number(f.reach)) ? Number(f.reach) : 5;
    const impact = Number.isFinite(Number(f.impact)) ? Number(f.impact) : 5;
    const confidence = Number.isFinite(Number(f.confidence)) ? Number(f.confidence) : 80;
    const effort = Number.isFinite(Number(f.effort)) && Number(f.effort) > 0 ? Number(f.effort) : 3;
    const computedScore = Math.round((reach * impact * confidence) / effort);
    const rice_score = Number.isFinite(Number(f.rice_score)) ? Number(f.rice_score) : computedScore;

    // Sanitize string and array fields from untrusted model output to prevent injection attacks
    const reasoning = typeof f.reasoning === 'string' ? f.reasoning.trim() : '';
    const category = typeof f.category === 'string' ? f.category.trim() : 'Other';
    const risks = Array.isArray(f.risks)
      ? f.risks.filter((r) => typeof r === 'string' && r.trim()).map((r) => r.trim())
      : [];

    return {
      ...f,
      name: String(f.name || 'Untitled Feature').trim(),
      reach,
      impact,
      confidence,
      effort,
      rice_score,
      sprint: String(f.sprint || 'LATER').toUpperCase(),
      reasoning,
      category,
      risks,
    };
